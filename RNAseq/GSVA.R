...
perform_gsva_multiple <- function(gene_sample_matrix, gene_sets, group_labels, pair_info) {
  # Validate inputs
  if (!is.matrix(gene_sample_matrix)) {
    stop("The input gene_sample_matrix must be a matrix.")
  }
  ...
  # Perform GSVA
  gsva_scores <- gsva(as.matrix(gene_sample_matrix), filtered_gene_sets, method = "gsva", verbose = FALSE)
  ...
  p <- ggplot(gsva_data, aes(x = GeneSet, y = GSVA_Score, fill = Group)) +
    ...
    theme_minimal() +
    scale_fill_manual(values = c("#4c72b0", "#c49c94")) + 
    scale_color_manual(values = c("#4c72b0", "#c49c94")) +
    theme(
      text = element_text(size = 18),
      plot.title = element_text(hjust = 0.5, face = "bold"),
      axis.text.x = element_text(angle = 45, hjust = 1),
